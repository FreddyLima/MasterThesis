...
class FloatSerializer(Serializer[float]):
    ...
    async def to_database(
        ...
        items = [DbFloat(
                         birth=f['birth'],
                         speed_y=f['speed_y'],
                         speed_y_pen=f['speed_y_pen'],
                         speed_y_joints=f['speed_y_joints'],
                         num_act_joints=f['num_act_joints'],
                         speed_x=f['speed_x'],
    ...
    async def from_database(cls, session: AsyncSession, ids: List[int]) -> List[float]:
        ...
        for i in range(len(items)):
            ...
            measures['speed_y_pen'] = items[i].speed_y_pen
            measures['speed_y_joints'] = items[i].speed_y_joints
            measures['num_act_joints'] = items[i].num_act_joints
...
class DbFloat(DbBase):
    ...
    speed_y_pen = sqlalchemy.Column(sqlalchemy.Float, nullable=True)
    speed_y_joints = sqlalchemy.Column(sqlalchemy.Float, nullable=True)
    num_act_joints = sqlalchemy.Column(sqlalchemy.Float, nullable=True)
