...
class Measure:
    ...
    def __init__(self, states=None, genotype_idx=-1, phenotype=None, generation=None,
                 simulation_time=None, env_conditions=None, n_joints_off=None):
        self._states = states
    ...
    def _speed_penalty(self):
        """
        Calculate speed with penalty based on num of joints.
        """
        if self._measures['hinge_prop'] <= 0:
            self._measures['speed_y_pen'] = 0
        elif self._measures['speed_y'] < 0:
            self._measures['speed_y_pen'] = self._measures['speed_y'] * self._measures['hinge_prop']
        else:
            self._measures['speed_y_pen'] = self._measures['speed_y'] / self._measures['hinge_prop']


    def _speed_joints(self):
        """
        Calculate speed with penalty based on num of active joints.
        """
        if self._joints_off.count(1) == 0:
            self._measures['speed_y_joints'] = 0
        elif self._measures['speed_y'] < 0:
            self._measures['speed_y_joints'] = self._measures['speed_y'] * self._joints_off.count(1)
        else:
            self._measures['speed_y_joints'] = self._measures['speed_y'] / self._joints_off.count(1)
    # controller measures

    def _save_joints(self):
        self._measures['num_act_joints'] = self._joints_off.count(1)
        self._measures['mask_joints'] = self._joints_off
