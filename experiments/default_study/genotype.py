...
#from revolve2.standard_resources.revolve2.standard_resources.modular_robots import *

from body_spider import *
from body_salamander import *
from body_snake import *
...
def mutate(
    ...
    mut_bod, _ = mutate_v1(genotype.body, _MULTINEAT_PARAMS_BODY, innov_db_body, multineat_rng, [[0,0,0,0]])
    mut_bra, joints_off = mutate_v1(genotype.brain, _MULTINEAT_PARAMS_BRAIN, innov_db_brain, multineat_rng, joints_off)
...
def develop(genotype: Genotype, querying_seed: int, max_modules: int, substrate_radius: str, env_condition: list,
            n_env_conditions: int, plastic_body: int, plastic_brain: int, loop: str, joints_off: List, body_phenotype: str) -> ModularRobot:
    ...
    if body_phenotype == 'salamander':
        body = make_body_salamander()
        queried_substrate = []

    if body_phenotype == 'snake':
        body = make_body_snake()
        queried_substrate = []
